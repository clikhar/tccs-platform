...
async def _history_rows(call_type: str | None, status: str | None, date_from: str | None, date_to: str | None, limit: int = 500):
    ...
    async with SessionLocal() as db:
        result = await db.execute(text(f"""
            ...
            ORDER BY originated_at DESC LIMIT :limit
        """), params)
...
def _serialize(row: dict) -> dict:
    ...
    return {**row, "originated_at": dt(row.get("originated_at")), "answered_at": dt(row.get("answered_at")),
...
async def export_history_csv(call_type: str | None = Query(None), status: str | None = Query(None),
    ...
    fields = ["id", "call_type", "source_extension", "target_station_number", "target_name", "group_code",
              "status", "originated_at", "answered_at", "ended_at", "duration_seconds"]
    stream = io.StringIO()
    ...
    return StreamingResponse(data, media_type="text/csv; charset=utf-8",
                             headers={"Content-Disposition": "attachment; filename=tccs-call-history.csv"})
...
async def export_history_pdf(call_type: str | None = Query(None), status: str | None = Query(None),
    ...
    width, height = page
    title = "TCCS CALL HISTORY REPORT"
    headers = ["TIME", "TYPE", "SOURCE", "TARGET", "STATUS", "ANSWERED", "ENDED", "DURATION"]
    x = [24, 145, 225, 295, 430, 510, 590, 680]
    def header():
        pdf.setFont("Helvetica-Bold", 10)
        pdf.drawString(24, height - 28, title)
        pdf.setFont("Helvetica", 7)
        filters = f"Type={call_type or 'ALL'}  Status={status or 'ALL'}  From={date_from or '—'}  To={date_to or '—'}"
        pdf.drawString(24, height - 42, filters[:180])
    ...
    for row in rows:
        if y < 28:
            pdf.showPage(); header(); y = height - 75; pdf.setFont("Helvetica", 6.5)
        target = f"{row.get('target_station_number') or ''} {row.get('target_name') or ''}".strip()
        values = [str(row.get("originated_at") or "")[:19].replace("T", " "), row.get("call_type") or "",
                  row.get("source_extension") or "", target[:25], row.get("status") or "",
                  str(row.get("answered_at") or "")[:19].replace("T", " "), str(row.get("ended_at") or "")[:19].replace("T", " "),
                  f"{int(row['duration_seconds'])}s" if row.get("duration_seconds") is not None else "—"]
    ...
    return StreamingResponse(output, media_type="application/pdf",
                             headers={"Content-Disposition": "attachment; filename=tccs-call-history.pdf"})
...
async def play_recording(filename: str, user: dict = Depends(require_user)):
    ...
    return FileResponse(path, media_type=media_type, headers={"Accept-Ranges": "bytes", "Cache-Control": "no-store",
