...
def _make_token(user_id: int, username: str, role: str, controller_id: Optional[int]) -> str:
    payload = {"sub": user_id, "username": username, "role": ROLE_ADMIN if role == "ADMIN" else role, "controller_id": controller_id, "exp": int(time.time()) + int(os.getenv("TCCS_ADMIN_TOKEN_TTL", "28800"))}
...
async def require_user(credentials: Optional[HTTPAuthorizationCredentials] = Depends(bearer), db: AsyncSession = Depends(get_db)) -> dict:
    ...
    payload.update({"id": row.id, "username": row.username, "role": role, "controller_id": row.controller_id})
    return payload
...
async def list_users(db: AsyncSession = Depends(get_db), admin: dict = Depends(require_admin)):
    result = await db.execute(text("SELECT u.id,u.username,CASE WHEN u.role='ADMIN' THEN 'ADMINISTRATOR' ELSE u.role END AS role,u.enabled,u.controller_id,c.code AS controller_code,c.name AS controller_name FROM admin_users u LEFT JOIN controllers c ON c.id=u.controller_id ORDER BY u.username"))
...
async def update_user(user_id:int,payload:dict=Body(...),db:AsyncSession=Depends(get_db),admin:dict=Depends(require_admin)):
    ...
    try:
        if password:
            await db.execute(text("UPDATE admin_users SET username=:username,password_hash=:password_hash,role=:role,enabled=:enabled,controller_id=:controller_id WHERE id=:id"),{"id":user_id,"username":username,"password_hash":_hash_password(str(password)),"role":role,"enabled":enabled,"controller_id":controller_id})
        else:
            await db.execute(text("UPDATE admin_users SET username=:username,role=:role,enabled=:enabled,controller_id=:controller_id WHERE id=:id"),{"id":user_id,"username":username,"role":role,"enabled":enabled,"controller_id":controller_id})
...
async def list_roles(admin:dict=Depends(require_admin)):
    return [
        {"code":ROLE_CONTROLLER,"name":"Controller","description":"Operating page for the assigned controller only","permissions":["OPERATING_ASSIGNED_CONTROLLER"]},
        {"code":ROLE_TESTROOM,"name":"Testroom","description":"Operating pages for all controllers and recording playback","permissions":["OPERATING_ALL_CONTROLLERS","RECORDINGS"]},
        {"code":ROLE_ADMIN,"name":"Administrator","description":"Full system administration and operating access","permissions":["ALL"]},
    ]
...
async def ensure_user_schema(db:AsyncSession)->None:
    ...
    await db.execute(text("CREATE INDEX IF NOT EXISTS idx_admin_users_controller ON admin_users(controller_id)"))
    await db.commit()
