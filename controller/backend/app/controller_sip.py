...
from .master import require_admin, router as master_router
from .user_management import require_user

router = APIRouter(prefix="", tags=["controller-sip"])
...
async def list_sip_accounts(db: AsyncSession = Depends(get_db), admin: dict = Depends(require_admin)):
    await ensure_controller_sip_tables(db)
...
async def create_sip_account(payload: dict = Body(...), db: AsyncSession = Depends(get_db), admin: dict = Depends(require_admin)):
    ...
    try:
        result = await db.execute(text("INSERT INTO sip_accounts(name,extension,username,password,enabled) VALUES(:name,:extension,:username,:password,:enabled) RETURNING id,name,extension,username,enabled"), {"name": name[:128], "extension": extension[:64], "username": username[:128], "password": password[:512], "enabled": bool(payload.get("enabled", True))})
        row = result.first(); await db.commit(); return dict(row._mapping)
    except Exception as exc:
        await db.rollback()
        if "duplicate" in str(exc).lower() or "unique" in str(exc).lower(): raise HTTPException(status_code=409, detail="SIP extension or username already exists")
        raise
...
async def update_sip_account(account_id: int, payload: dict = Body(...), db: AsyncSession = Depends(get_db), admin: dict = Depends(require_admin)):
    ...
    if current is None: raise HTTPException(status_code=404, detail="SIP account not found")
    name = str(payload.get("name", current.name) or "").strip(); extension = str(payload.get("extension", current.extension) or "").strip(); username = str(payload.get("username", current.username) or "").strip(); password = str(payload.get("password") or current.password)
    if not name or not extension or not username or len(password) < 6: raise HTTPException(status_code=400, detail="Valid SIP name, extension, username and password are required")
    try:
        result = await db.execute(text("UPDATE sip_accounts SET name=:name,extension=:extension,username=:username,password=:password,enabled=:enabled WHERE id=:id RETURNING id,name,extension,username,enabled"), {"id": account_id, "name": name[:128], "extension": extension[:64], "username": username[:128], "password": password[:512], "enabled": bool(payload.get("enabled", current.enabled))})
        row = result.first(); await db.commit(); return dict(row._mapping)
    except Exception as exc:
        await db.rollback()
        if "duplicate" in str(exc).lower() or "unique" in str(exc).lower(): raise HTTPException(status_code=409, detail="SIP extension or username already exists")
...
async def delete_sip_account(account_id: int, db: AsyncSession = Depends(get_db), admin: dict = Depends(require_admin)):
    ...
    if row is None: raise HTTPException(status_code=404, detail="SIP account not found")
...
async def assign_controller_sip(controller_id: int, payload: dict = Body(...), db: AsyncSession = Depends(get_db), admin: dict = Depends(require_admin)):
    ...
    raw = payload.get("sip_account_id"); account_id: Optional[int] = None
    if raw not in (None, ""):
        try: account_id = int(raw)
        except (TypeError, ValueError): raise HTTPException(status_code=400, detail="Invalid SIP account")
        account = (await db.execute(text("SELECT id,enabled FROM sip_accounts WHERE id=:id"), {"id": account_id})).first()
        if account is None or not account.enabled: raise HTTPException(status_code=400, detail="SIP account not found or disabled")
        ...
        if used is not None: raise HTTPException(status_code=409, detail=f"SIP account is already assigned to controller {used.code}")
    await db.execute(text("UPDATE controllers SET sip_account_id=:account_id WHERE id=:id"), {"id": controller_id, "account_id": account_id}); await db.commit()
    return {"status": "ASSIGNED", "controller_id": controller_id, "sip_account_id": account_id}

@router.get("/available-controllers/{controller_id}/sip-credentials")
async def controller_sip_credentials(controller_id: int, db: AsyncSession = Depends(get_db), user: dict = Depends(require_user)):
    await ensure_controller_sip_tables(db)
    if user.get("role") == "CONTROLLER" and int(user.get("controller_id") or 0) != controller_id: raise HTTPException(status_code=403, detail="Controller access is restricted to the assigned controller")
    result = await db.execute(text("SELECT c.id AS controller_id,c.code,c.name,c.section_id,a.id AS sip_account_id,a.extension,a.username,a.password,a.enabled AS sip_enabled FROM controllers c LEFT JOIN sip_accounts a ON a.id=c.sip_account_id WHERE c.id=:id AND c.enabled=TRUE"), {"id": controller_id})
    row = result.first()
    if row is None: raise HTTPException(status_code=404, detail="Controller not found or disabled")
    if row.sip_account_id is None or not row.sip_enabled: raise HTTPException(status_code=404, detail="No enabled SIP account is assigned to this controller")
    return dict(row._mapping)

# Mount these endpoints below the existing /api/v1/master prefix.
