...
async function verify(){const t=token();if(!t){location.replace('/');return null}try{const r=await fetch(API+'/v1/master/auth/me',{headers:{Authorization:'Bearer '+t}});if(!r.ok)throw Error();const u=await r.json();localStorage.setItem(userKey,JSON.stringify(u));return u}catch{localStorage.removeItem(tokenKey);localStorage.removeItem(userKey);location.replace('/');return null}}
