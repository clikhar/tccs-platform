...
if(u&&url.includes('/api/v1/stations')&&!url.includes('/order')&&response.ok){try{const data=await response.clone().json();let sectionId=null;if(u.role==='CONTROLLER'){const selected=await controllerContext();sectionId=selected&&selected.section_id}else if(location.pathname==='/controller.html'){const selected=await controllerContext();sectionId=selected&&selected.section_id}
